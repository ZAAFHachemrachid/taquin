...
impl Solver for BFSSolver {
    fn solve(&self, optimal_length: Option<usize>) -> Option<SolutionInfo> {
        ...
        let max_depth = 20; // Limit search depth to avoid long-running searches
        ...
        while let Some(current_state) = stack.pop() {
            ...
            // Collect and sort next states by path length
            ...
            // Sort in ascending order by path length since we pop from the end
            // This helps maintain breadth-first behavior with a stack
            next_states.sort_by_key(|state| state.path.len());
